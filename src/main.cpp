...
$execute{
	...
	// register the new blocks tab
	EditorTabs::get()->registerTab(TabType::BUILD, "newBlocks"_spr, [](EditorUI* ui, CCMenuItemToggler* toggler)->CCNode* {
	...
	// register the new slopes tab
	EditorTabs::get()->registerTab(TabType::BUILD, "newSlopes"_spr, [](EditorUI* ui, CCMenuItemToggler* toggler)->CCNode* {
		auto items = CCArray::create();
	...
	//other new tabs: outlines/3DL, hazards/sawblades, pixel, deco, triggers
	// register the tab with 2 new outlines and some modified 3D blocks
	EditorTabs::get()->registerTab(TabType::BUILD, "NewOutlines"_spr, [](EditorUI* ui, CCMenuItemToggler* toggler)->CCNode* {
		auto items = CCArray::create();
	...
	// register the tab with additional hazards: saws and spikes
	EditorTabs::get()->registerTab(TabType::BUILD, "NewHazards"_spr, [](EditorUI* ui, CCMenuItemToggler* toggler)->CCNode* {
		auto items = CCArray::create();
	...
	// register a tab with the strange set of way-too-many pixel objects
	//EditorTabs::get()->registerTab(TabType::BUILD, "NewPixels"_spr, [](EditorUI* ui, CCMenuItemToggler* toggler)->CCNode* {
		//auto items = CCArray::create();
		...
		*/
		...
		//}, [](EditorUI*, bool state, CCNode*) {
	//});
	// register the tab for additional decoration objects
	EditorTabs::get()->registerTab(TabType::BUILD, "newDeco"_spr, [](EditorUI* ui, CCMenuItemToggler* toggler)->CCNode* {
		auto items = CCArray::create();
	...
	// register the unusual objects tab
	EditorTabs::get()->registerTab(TabType::BUILD, "UnusalObjs"_spr, [](EditorUI* ui, CCMenuItemToggler* toggler)->CCNode* {
		auto items = CCArray::create();
	...
	// register the new triggers tab
	EditorTabs::get()->registerTab(TabType::BUILD, "newTriggers"_spr, [](EditorUI* ui, CCMenuItemToggler* toggler)->CCNode* {
		auto items = CCArray::create();
...
// EXTERNALS geode.node-ids:1.13.1 alphalaneous.editortab_api:1.0.0-alpha.10
bool block;
bool outline;
bool slope;
bool hazard;
